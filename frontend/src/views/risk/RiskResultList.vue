<template>
  <div class="risk-result-page">
    <PageHeader
      title="风险审查结果"
      subtitle="按审批单ID、原文档名称或任务ID查询风险审查结果，并支持查看与下载风险报告"
    />
      ...
      <a-form layout="inline" :model="searchForm" @finish="handleSearch">
        ...
        <a-form-item>
          <a-space>
            ...
            <a-button type="default" :disabled="!currentApprovalOrderId" @click="handleViewReport">
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'action'">
            <a-space>
              <a-button type="link" size="small" :disabled="record.is_valid === true" @click="handleValidate(record, true)">
                确认
              </a-button>
              <a-button type="link" size="small" danger :disabled="record.is_valid === false" @click="handleValidate(record, false)">
                驳回
              </a-button>
            </a-space>
          </template>
          ...
          <div class="report-toolbar">
            <a-space compact>
              <a-input
                ...
                placeholder="输入审批单ID、原文档名称或风险关键字"
                allow-clear
                style="width: 520px"
                @pressEnter="handleReportSearch"
              />
              <a-button @click="handleReportSearch">搜索</a-button>
...
const columns: TableColumnsType<RiskResult> = [
  ...
  { title: '原文档名称', dataIndex: 'field_name', key: 'field_name', width: 120, ellipsis: true },
...
async function handleReportSearch() {
  const keyword = reportKeyword.value.trim()
  if (!keyword) {
    reportFilterKeyword.value = ''
    return
  }
  ...
  try {
    ...
    if (!nextApprovalOrderId || nextApprovalOrderId === currentApprovalOrderId.value) {
